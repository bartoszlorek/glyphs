...
function validation(range) {
    ...
    // bug: Adding Glyph to the empty message in (commonly used)
    ...
    if (range.collapsed && container.children && container.children.length) {
        ...
        if (name !== null) {
            return result(
                `There is a limitation which mainly occurs in ${name}. Adding Glyph to the empty message causes an error. Write something and then add Glyph. Sorry for the inconvenience.`
            )
        }
    }
